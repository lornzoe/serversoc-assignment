...
#region Operations and Connection Methods
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        //ASSIGNMENT
        case PunEvent.LoginCheck:
            {
                LoginCheck loginCheck = LoginCheck.Deserialize((byte[])photonEvent.Parameters[photonEvent.Code]);
                    
                if (loginCheck.command == "WrongPassword")
                {
                    GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
                    foreach (GameObject player in players)
                    {
                        if (player.GetComponent<PhotonView>().owner.NickName.Equals(loginCheck.playerName))
                        {
                            GameObject.FindGameObjectWithTag("Code").GetComponent<GameManagerVik>().loginOk = loginCheck.command;
                            GameObject.FindGameObjectWithTag("DataPasser").GetComponent<DataPasser>().MainMenuLoginOK = loginCheck.command;
                        }
                    }    
                }
                else if(loginCheck.command == "newplayer")
                {
                    GameObject.FindGameObjectWithTag("DataPasser").GetComponent<DataPasser>().MainMenuLoginOK = "";
                    DataPasser.Instance.gamesparksObject.GetComponent<GamesparksLoginUI>().UserAuthenticate(loginCheck.playerName, DataPasser.Instance.playerPassword);
                }
                else if(loginCheck.command == "LoginOK")
                {
                    GameObject.FindGameObjectWithTag("DataPasser").GetComponent<DataPasser>().MainMenuLoginOK = "";
                    ...
                    temp.playerName = loginCheck.playerName;
                    temp.command = "LoginOKGimmeData";

                    PhotonNetwork.RaiseEvent(evCode, LoginCheck.Serialize(temp), reliable, null);
                }
            }
            break;
        case PunEvent.PlayerData:
            {
                PlayerDataInfo pData = PlayerDataInfo.Deserialize((byte[])photonEvent.Parameters[photonEvent.Code]);

                GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
                foreach (GameObject player in players)
                {
                    if (player.GetComponent<PhotonView>().owner.NickName.Equals(pData.Username))
                    {
                        //Debug.Log("Changed: " + player.GetComponent<PhotonView>().owner.NickName);
                        player.transform.position = new Vector3(pData.posX, pData.posY, pData.posZ);
                        player.transform.eulerAngles = new Vector3(pData.rotX, pData.rotY, pData.rotZ);
                        player.transform.Find("Main Camera").localPosition = new Vector3(pData.cameraPosX, pData.cameraPosY, pData.cameraPosZ);
                        player.transform.Find("Main Camera").localEulerAngles = new Vector3(pData.cameraRotX, pData.cameraRotY, pData.cameraRotZ);

                            //Player Items
                        if (pData.item1Name != "NOTHING")
                        {
                            ItemData temp = new ItemData();
                            temp.itemName = pData.item1Name;
                            temp.attack = pData.item1Attack;

                            player.GetComponent<PlayerData>().Inventory.Add(temp);
                                
                            if (pData.item2Name != "NOTHING")
                            {
                                ItemData temp2 = new ItemData();
                                temp2.itemName = pData.item2Name;
                                temp2.attack = pData.item2Attack;

                                player.GetComponent<PlayerData>().Inventory.Add(temp2);
                            }
                        }

                        //Player FriendList
                        if (pData.friend1Name != "NOTHING")
                        {
                            player.GetComponent<PlayerData>().friendList.Add(pData.friend1Name);
                                
                            if (pData.friend2Name != "NOTHING")
                            {
                                player.GetComponent<PlayerData>().friendList.Add(pData.friend2Name);
                                    
                                if (pData.friend3Name != "NOTHING")
                                {
                                    player.GetComponent<PlayerData>().friendList.Add(pData.friend3Name);
                                }
                            }
                        }

                        //Player Pending FriendList
                        if (pData.pending1Name != "NOTHING")
                        {
                            player.GetComponent<PlayerData>().pendingFriendList.Add(pData.pending1Name);
                                
                            if (pData.pending2Name != "NOTHING")
                            {
                                player.GetComponent<PlayerData>().pendingFriendList.Add(pData.pending2Name);
                                    
                                if (pData.pending3Name != "NOTHING")
                                {
                                    player.GetComponent<PlayerData>().pendingFriendList.Add(pData.pending3Name);
                                }
                            }
                        }

                        DataPasser.Instance.gamesparksObject.GetComponent<GamesparksLoginUI>().UserAuthenticate(pData.Username, DataPasser.Instance.playerPassword);
                        player.GetComponent<PlayerData>().LoadPlayerKillsGamespark();
                        player.GetComponent<PlayerData>().LoadAchivements();
                        break;
                    }
                }
                ...
                {
                    if (pet.GetComponent<PhotonView>().owner.NickName.Equals(pData.Username))
                    {
                        pet.transform.position = new Vector3(pData.petPosX, pData.petPosY, pData.petPosZ);
                        pet.transform.eulerAngles = new Vector3(pData.petRotX, pData.petRotY, pData.petRotZ);
                        break;
                    }
                }
                break;
            }
        case PunEvent.UpdateAI:
                {
                    GameObject[] Enemies = GameObject.FindGameObjectsWithTag("EnemyParentObject");
                    foreach (GameObject enemy in Enemies)
                    {
                        enemy.GetComponent<Enemy>().AIUpdate();
                    }
                }
                break;
        default:
    ...
    public Vector3 GetVector3FromString(string input, string tag)
    {
        //Get tag
        string outputString = "";
        ...
        var sStrings = outputString.Split("_"[0]);
