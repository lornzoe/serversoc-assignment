            ...
            case PunEvent.PlayerLogin:
                {
                    ...
                    {
                        GameObject.FindGameObjectWithTag("Code").GetComponent<GameManagerVik>().loginCheck = dataString;
                        GameObject.FindGameObjectWithTag("DataPasser").GetComponent<DataPasser>().mainMenuPasser = dataString;

                    }
                    else if (dataString == "newPlayer")
                    {
                        GameObject.FindGameObjectWithTag("Code").GetComponent<GameManagerVik>().loginCheck = "";
                    }
                    else // regular login here
                    {
                        GameObject.FindGameObjectWithTag("DataPasser").GetComponent<DataPasser>().mainMenuPasser = "";

                        GameObject[] players = GameObject.FindGameObjectsWithTag("Player");

                        foreach (GameObject player in players)
                        {
                            string playerName = GetSubstringFromString(dataString, "playerName");

                            if (player.GetComponent<PhotonView>().owner.NickName.Equals(playerName))
                            {
                                Debug.Log("Changed: " + player.GetComponent<PhotonView>().owner.NickName);
                                player.transform.position = GetVector3FromString(dataString, "position");
